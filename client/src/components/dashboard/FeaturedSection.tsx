...
function FeaturedTile(props:{title:string, canteen:string, price:string, time:string, image:string,count:number,description?:string})
{
    return (
        <div style={{width:`${100/props.count}%`}} className="h-full flex flex-col justify-between relative md:p-16 p-8">
            <div className="absolute -z-10 top-0 left-0 w-full h-full bg-black">
                <img className="w-full h-full object-cover opacity-70" src={props.image} alt="" />
            </div>
            <div>
            ...
            <div>
                ...
                <div className="flex gap-4">
                    <Button className="mt-4" color={"primary"}>Order Now</Button>
                    <Button className="mt-4" color={"secondary"}>Add To Cart</Button>
                </div>
            </div>
        </div>
    )
